-- Force Create user_settings table
-- Migration 002 shows as applied but table doesn't exist
-- This migration ensures the table is created

-- Drop table if it exists in wrong state, then recreate
...
DROP POLICY IF EXISTS "Public can upsert user settings" ON user_settings;
