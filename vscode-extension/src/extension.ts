...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('alexAi.chatView', chatViewProvider)
  );
  ...
  vscode.window.registerTreeDataProvider('alexAi.sprintView', sprintTreeProvider);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('alexAi.askCrew', async () => {
      ...
      const crewMember = await vscode.window.showQuickPick(
        [
          { label: '🎖️ Captain Picard', value: 'picard', description: 'Strategic advice' },
          { label: '⚡ Commander Riker', value: 'riker', description: 'Tactical assessment' },
          { label: '🤖 Commander Data', value: 'data', description: 'Technical analysis' },
          { label: '🔧 Lt. Cmdr. La Forge', value: 'geordi', description: 'Engineering' },
          { label: '💭 Counselor Troi', value: 'troi', description: 'UX perspective' },
          { label: '⚔️ Lt. Worf', value: 'worf', description: 'Security review' },
          { label: '🛠️ Chief O\'Brien', value: 'obrien', description: 'Implementation' },
          { label: '💰 Quark', value: 'quark', description: 'Business analysis' },
        ],
        { placeHolder: 'Select a crew member to ask' }
      );

      if (crewMember) {
        chatViewProvider.askCrewAboutCode(crewMember.value, selection, editor.document.languageId);
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('alexAi.explainCode', async () => {
      ...
      chatViewProvider.askCrewAboutCode('data', selection, editor.document.languageId, 'explain');
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('alexAi.reviewCode', async () => {
      ...
      chatViewProvider.askCrewAboutCode('worf', selection, editor.document.languageId, 'review');
    })
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('alexAi.optimizeCode', async () => {
      ...
      chatViewProvider.askCrewAboutCode('data', selection, editor.document.languageId, 'optimize');
    })
  );
