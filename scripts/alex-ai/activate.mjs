...
const CREW_MEMBERS = [
  { id: 'captain_picard', name: 'Captain Picard', icon: '🎖️', role: 'Strategic Leadership' },
  { id: 'commander_riker', name: 'Commander Riker', icon: '⚡', role: 'Tactical Coordination' },
  { id: 'commander_data', name: 'Commander Data', icon: '🤖', role: 'Technical Analysis' },
  { id: 'geordi_la_forge', name: 'Lt. Cmdr. La Forge', icon: '🔧', role: 'Infrastructure' },
  { id: 'counselor_troi', name: 'Counselor Troi', icon: '💭', role: 'UX Design' },
  { id: 'lieutenant_worf', name: 'Lt. Worf', icon: '⚔️', role: 'Security' },
  { id: 'dr_crusher', name: 'Dr. Crusher', icon: '💊', role: 'System Health' },
  { id: 'chief_obrien', name: "Chief O'Brien", icon: '🛠️', role: 'Implementation' },
  { id: 'quark', name: 'Quark', icon: '💰', role: 'Business Strategy' },
  { id: 'lieutenant_uhura', name: 'Lt. Uhura', icon: '📻', role: 'Communication' },
];
...
async function main() {
  ...
  console.log('╔═══════════════════════════════════════════════════════════════╗');
  console.log('║                                                               ║');
  console.log('║   🖖  ALEX AI CREW SYSTEM - ACTIVATED                         ║');
  console.log('║                                                               ║');
  console.log('╚═══════════════════════════════════════════════════════════════╝');
  console.log('\n');
  ...
  for (const member of CREW_MEMBERS) {
    const memberMemories = memories.filter(m => m.crewId === member.id).length;
    console.log(`  ${member.icon} ${member.name.padEnd(22)} │ ${member.role.padEnd(20)} │ ${memberMemories} memories`);
  }
  console.log('─'.repeat(60));
  console.log(`  Total: ${CREW_MEMBERS.length} crew members | ${memories.length} memories loaded\n`);
