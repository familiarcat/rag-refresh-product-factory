...
async function runToCompletion(sprintId) {
  ...
  if (!sprintId) {
    const res = await fetch(`${BASE_URL}/api/sprints?projectId=${DEFAULT_PROJECT}`);
    const data = await res.json();
    const activeSprint = data.sprints?.find(s => s.status === 'active' || s.status === 'planning');
  ...
  while (!complete && cycle <= 20) {
    ...
    try {
      ...
      console.log(`   ✅ ${summary.done} done | 🚀 ${summary.inProgress} in progress | 👀 ${summary.review} review | 🚫 ${summary.blocked} blocked`);

      // Show completed stories this cycle
      const completed = data.cycleResults?.filter(r => r.newStatus === 'done') || [];
