...
async function getSprintStatus(projectId) {
  try {
    ...
    console.log(`
${statusEmoji[sprint.status] || '📦'} ${sprint.name} - ${sprint.status.toUpperCase()}
${'═'.repeat(50)}
...
📊 Progress
   ...
   Progress:  ${sprint.committedPoints > 0 ? Math.round((sprint.completedPoints / sprint.committedPoints) * 100) : 0}%
...
`);
    ...
    for (const status of statusOrder) {
      if (statusGroups[status]) {
        const bar = '█'.repeat(statusGroups[status]) + '░'.repeat(Math.max(0, 10 - statusGroups[status]));
        console.log(`   ${statusIcons[status]} ${status.padEnd(12)} ${bar} ${statusGroups[status]}`);
      }
    }
