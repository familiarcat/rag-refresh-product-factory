  ...
  return (
    <div className="grid">
      <div className="card span-12">
        <h1 style={{marginTop:0}}>Create</h1>
        <p className="small">
        ...
        <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
          <input value={topic} onChange={e=>{setTopic(e.target.value);}} onBlur={()=>regenerate()} style={{flex:1,minWidth:260,padding:10,borderRadius:12,border:'1px solid rgba(255,255,255,.18)',background:'rgba(255,255,255,.04)',color:'var(--text)'}}/>
          <select value={tpl} onChange={e=>{setTpl(e.target.value); regenerate(topic, e.target.value);}} style={{padding:10,borderRadius:12,border:'1px solid rgba(255,255,255,.18)',background:'rgba(255,255,255,.04)',color:'var(--text)'}}>
            {templates.map(t => <option key={t.name} value={t.name}>{t.name}</option>)}
          </select>
          <button onClick={()=>regenerate()} style={{padding:'10px 14px',borderRadius:12,border:'1px solid rgba(255,255,255,.18)',background:'rgba(255,255,255,.06)',color:'var(--text)'}}>Regenerate</button>
          <button onClick={save} disabled={saving} style={{padding:'10px 14px',borderRadius:12,border:'1px solid rgba(40,217,154,.35)',background:'rgba(40,217,154,.08)',color:'var(--ok)'}}>{saving?'Saving…':'Save note'}</button>
        </div>
      </div>
      <div className="card span-12">
        <h2 style={{marginTop:0}}>Draft</h2>
        <textarea value={draft} onChange={e=>setDraft(e.target.value)} style={{width:'100%',minHeight:320,padding:12,borderRadius:12,border:'1px solid rgba(255,255,255,.18)',background:'rgba(0,0,0,.25)',color:'var(--text)',fontFamily:'ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, Liberation Mono, Courier New'}}/>
      </div>
    </div>
  );
}
