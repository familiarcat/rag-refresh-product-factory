...
export default function ProjectDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'domains' | 'crew' | 'settings'>('overview');
  ...
  function DomainCard({ domain, onUpdate }: { domain: ProjectDomain; onUpdate: (d: ProjectDomain) => void }) {
    const [editing, setEditing] = useState(false);

    return (
      <div className="card" style={{ borderLeft: `3px solid ${getScoreColor(domain.scores.demand)}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h3 style={{ margin: 0, fontSize: 16 }}>{domain.name}</h3>
            <p className="small" style={{ margin: '4px 0', color: 'var(--muted)' }}>
              {domain.description}
            </p>
          </div>
          <span style={{
            ...
            borderRadius: 4,
            fontSize: 11,
            background: domain.status === 'completed' ? '#10b98120' : 
                       domain.status === 'in-progress' ? '#3b82f620' : '#6b728020',
            color: domain.status === 'completed' ? '#10b981' : 
                   domain.status === 'in-progress' ? '#3b82f6' : '#6b7280',
        ...
        <div style={{ 
          ...
          {Object.entries(domain.scores).map(([key, value]) => (
            <div key={key}>
              <div style={{ color: getScoreColor(value), fontWeight: 'bold' }}>{value}</div>
              <div style={{ color: 'var(--muted)', textTransform: 'capitalize' }}>{key.slice(0, 3)}</div>
            </div>
          ))}
        </div>
        ...
        {domain.features.length > 0 && (
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
            <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>Features:</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card" style={{ 
        background: `linear-gradient(135deg, var(--surface) 0%, ${getStatusColor(project.status)}15 100%)`,
        borderLeft: `4px solid ${getStatusColor(project.status)}`,
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
              <Link href="/projects" style={{ color: 'var(--muted)', fontSize: 12 }}>
                ← Projects
              </Link>
            </div>
            ...
            <p style={{ margin: '8px 0 0', color: 'var(--muted)', maxWidth: 600 }}>
              {project.tagline}
            </p>
            {category && (
              <div style={{ 
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {(['draft', 'active', 'paused', 'completed'] as ProjectStatus[]).map(status => (
              <button
                key={status}
                ...
                style={{
                  padding: '6px 12px',
                  border: project.status === status ? `2px solid ${getStatusColor(status)}` : '2px solid transparent',
                  borderRadius: 6,
                  background: project.status === status ? `${getStatusColor(status)}20` : 'var(--surface)',
                  color: project.status === status ? getStatusColor(status) : 'var(--muted)',
                  cursor: 'pointer',
                  fontSize: 11,
                  textTransform: 'capitalize',
                }}
      ...
      <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid var(--border)', paddingBottom: 8 }}>
        {(['overview', 'domains', 'crew', 'settings'] as const).map(tab => (
          <button
            key={tab}
        ...
        <>
          ...
          <div className="card">
            ...
            <div style={{ 
              ...
              <ScoreCard label="Demand" value={project.scores.demand} description="Market need" />
              <ScoreCard label="Effort" value={project.scores.effort} description="Build complexity" />
              <ScoreCard label="Monetization" value={project.scores.monetization} description="Revenue potential" />
              <ScoreCard label="Differentiation" value={project.scores.differentiation} description="Competitive edge" />
              <ScoreCard label="Risk" value={project.scores.risk} description="Uncertainty level" />
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            <div className="card" style={{ textAlign: 'center', padding: 16 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 'bold' }}>{project.domains.length}</div>
              <div className="small" style={{ color: 'var(--muted)' }}>Domains</div>
            </div>
            <div className="card" style={{ textAlign: 'center', padding: 16 }}>
              <div style={{ fontSize: 24, marginBottom: 4 }}>✅</div>
              <div style={{ fontSize: 20, fontWeight: 'bold' }}>{project.completedFeatures.length}</div>
              <div className="small" style={{ color: 'var(--muted)' }}>Completed</div>
            </div>
            <div className="card" style={{ textAlign: 'center', padding: 16 }}>
              <div style={{ fontSize: 24, marginBottom: 4 }}>🎯</div>
              <div style={{ fontSize: 20, fontWeight: 'bold' }}>{project.mvpFeatures.length}</div>
              <div className="small" style={{ color: 'var(--muted)' }}>MVP Features</div>
            </div>
            <div className="card" style={{ textAlign: 'center', padding: 16 }}>
              <div style={{ fontSize: 24, marginBottom: 4 }}>👥</div>
              <div style={{ fontSize: 20, fontWeight: 'bold' }}>{project.crew.length}</div>
              <div className="small" style={{ color: 'var(--muted)' }}>Crew</div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div className="card">
              <h3 style={{ margin: '0 0 12px', fontSize: 14 }}>🛠️ Tech Stack</h3>
            ...
            <div className="card">
              <h3 style={{ margin: '0 0 12px', fontSize: 14 }}>💰 Monetization</h3>
              <div style={{ fontSize: 13 }}>
                <div style={{ marginBottom: 8 }}>
                  <span style={{ color: 'var(--muted)' }}>Model: </span>
                  <span style={{ textTransform: 'capitalize' }}>{project.monetization.model}</span>
                </div>
                <div style={{ marginBottom: 8 }}>
                ...
                {project.monetization.revenueStreams.length > 0 && (
                  <div>
                    <div style={{ color: 'var(--muted)', marginBottom: 4 }}>Revenue Streams:</div>
          ...
          {project.description && (
            <div className="card">
              <h3 style={{ margin: '0 0 12px', fontSize: 14 }}>📝 Description</h3>
      ...
      {activeTab === 'crew' && (
        <div className="card">
          <h2 style={{ margin: '0 0 16px', fontSize: 16 }}>👥 Crew Assignments</h2>
          {project.crew.length === 0 ? (
            <p style={{ color: 'var(--muted)' }}>No crew members assigned yet.</p>
          ) : (
      ...
      {activeTab === 'settings' && (
        <div className="card">
          <h2 style={{ margin: '0 0 16px', fontSize: 16 }}>⚙️ Project Settings</h2>
