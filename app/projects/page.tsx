  ...
  function ScoreBar({ label, value }: { label: string; value: number }) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11 }}>
        ...
        <span style={{ width: 16, textAlign: 'right', color: 'var(--muted)' }}>{value}</span>
      </div>
    );
  }
      ...
      >
        ...
        >
          ...
          <div style={{ 
            ...
            <span>{getCategoryName(project.primaryCategory).split(' ')[0]}</span>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <div className="card" style={{ flex: 1, textAlign: 'center', padding: 16 }}>
          <div style={{ fontSize: 28, fontWeight: 'bold', color: '#10b981' }}>{activeCount}</div>
          <div className="small" style={{ color: 'var(--muted)' }}>Active</div>
        </div>
        <div className="card" style={{ flex: 1, textAlign: 'center', padding: 16 }}>
          <div style={{ fontSize: 28, fontWeight: 'bold', color: '#6b7280' }}>{draftCount}</div>
          <div className="small" style={{ color: 'var(--muted)' }}>Drafts</div>
        </div>
        <div className="card" style={{ flex: 1, textAlign: 'center', padding: 16 }}>
          <div style={{ fontSize: 28, fontWeight: 'bold', color: 'var(--accent)' }}>{projects.length}</div>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        {['all', 'active', 'draft', 'paused', 'completed', 'archived'].map(status => (
          <button
            key={status}
            ...
            style={{
              ...
              border: 'none',
              borderRadius: 6,
              background: filter === status ? 'var(--accent)' : 'var(--surface)',
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
          ...
          <div>
            ...
            <p className="small" style={{ margin: '8px 0 0', color: 'var(--muted)' }}>
              The Product Factory itself is managed as a project. Each generated project inherits 
              the same dashboard structure, enabling recursive self-improvement and consistent 
              architecture across the portfolio.
            </p>
            <p className="small" style={{ margin: '8px 0 0', opacity: 0.7 }}>
              💭 <em>&quot;The factory that builds factories, building itself.&quot; — Captain Picard</em>
            </p>
          </div>
        </div>
      </div>
