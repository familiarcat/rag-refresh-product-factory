...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    ...
    test_users: users,
    available_actions: {
      create_api_key: '/api/dev/test-auth?action=create-api-key&email=dev1@example.com',
    },
...
export async function POST(req: NextRequest) {
  ...
  const permissions = {
    'project:create': await checkPermission(user!.id, 'project:create'),
    'project:read': await checkPermission(user!.id, 'project:read'),
    'project:write': await checkPermission(user!.id, 'project:write'),
    'project:delete': await checkPermission(user!.id, 'project:delete'),
    'code:read': await checkPermission(user!.id, 'code:read'),
    'code:write': await checkPermission(user!.id, 'code:write'),
    'code:execute': await checkPermission(user!.id, 'code:execute'),
    'system:manage_users': await checkPermission(user!.id, 'system:manage_users'),
  };
